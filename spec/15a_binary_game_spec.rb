...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        letter_2 = 'b'
        valid = '2'
        allow(game_input).to receive(:gets).and_return(letter_1, letter_2, valid)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game_verify.instance_variable_get(:@maximum)
        invalid_input = 13
        nil_input = game_verify.verify_input(min, max, invalid_input)
        expect(nil_input).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(binary_search) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess).once
      game_turn.display_turn_order(binary_search)
    end
